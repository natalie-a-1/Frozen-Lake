...
def train_sarsa(params, runs=50):
    all_rewards = []
...
def train_sarsa2(params, runs=50):
...
params = SARSALearningParams(use_dynamic_lr=False)  # Set learning parameters
mean_rewards, rewards_std = train_sarsa(params)  # Train SARSA
smoothed_constant = smooth_rewards(mean_rewards, window_size=100)
...
def update_qtable_policy_gradient(state, action, reward, new_state, discount_factor, learning_rate):
    qtable[state, action] += learning_rate * (reward + discount_factor * (qtable[new_state, choose_action_policy_gradient(new_state, qtable)]) - qtable[state, action])
...
#                 action = choose_action_policy_gradient(state, qtable)  # Use policy gradient for action selection
                ...
                update_qtable_policy_gradient(int(state), action, reward, int(new_state), params.discount_factor, params.learning_rate)  # Update Q-table using policy gradient
    ...
    rewards_mean = np.mean(all_rewards, axis=0)
    rewards_std = np.std(np.mean(all_rewards, axis=1))
...
def train_sarsa3(params, runs=30):
...
plot_learning_curves(
    [smoothed_constant1, smoothed_constant2, smoothed_constant_eg],
    ['ALG-1', 'ALG-2', 'epsilon-greedy'],
